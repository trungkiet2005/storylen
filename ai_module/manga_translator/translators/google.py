...
DEFAULT_FALLBACK_SERVICE_URLS = ['translate.google.com']
TRANSLATE_RPC_URL = getattr(
    urls,
    'TRANSLATE_RPC',
    'https://{host}/_/TranslateWebserverUi/data/batchexecute'
)
...
def _create_async_client(http2: bool):
    client_args = {
        'http2': http2,
    }
    if SYS_HTTP_PROXY is not None:
        client_args['proxies'] = SYS_HTTP_PROXY
    ...
    client_args.pop('proxies', None)
    proxy = SYS_HTTP_PROXY
...
class GoogleTranslator(CommonTranslator):
    ...
    def __init__(self, service_urls=DEFAULT_CLIENT_SERVICE_URLS, user_agent=DEFAULT_USER_AGENT,
        ...
        self.client = _create_async_client(http2=http2)
    ...
    async def _request_and_parse_translation(self, query, to_lang, from_lang):
        ...
        if not resp:
            return await self._request_and_parse_legacy_translation(origin, to_lang, from_lang)
    ...
    async def _request_and_parse_legacy_translation(self, query, to_lang, from_lang):
        data, response = await self._translate_legacy(query, to_lang, from_lang, {})
        ...
        return Translated(
            ...
            parts=translated_parts,
            extra_data={'confidence': None, 'parts': translated_parts, 'parsed': data},
            response=response,
        )
    ...
    async def _request_translation(self, text: str, dest: str, src: str):
        url = TRANSLATE_RPC_URL.format(host=self._pick_service_url())
        data = {
    ...
    async def _translate_legacy(self, text, dest, src, override):
        ...
        client_type = 'gtx' if self.client_type == 'tw-ob' else self.client_type
        params = utils.build_params(client=client_type, query=text, src=src, dest=dest,
