...
\textbf{GVHD} & GS.TS Nguyễn Văn Vũ\\[3pt]
...
\A Supabase đã cung cấp sẵn PostgreSQL, nên bật extension pgvector là có ngay tìm kiếm vector --- \textbf{không phải vận hành thêm một dịch vụ}. Điều này giảm chi phí hạ tầng và độ phức tạp cho một MVP. Tài liệu PA ban đầu dự kiến ChromaDB; nhóm đã đổi sang pgvector khi hiện thực (xem Mục 10).
\end{qa}
...
\A \texttt{profiles}, \texttt{manga\_series}, \texttt{manga\_chapters} (cờ \texttt{published\_at} cho thư viện), \texttt{manga\_pages}, \texttt{bubble\_data} (bbox, text, review\_status), \texttt{embeddings} (pgvector), \texttt{qa\_history}, \texttt{credit\_transactions}; cùng các bảng cộng đồng (forum, comments), thông báo, share links. Quan hệ: series \arr\ chapters \arr\ pages \arr\ bubbles/embeddings.
\end{qa}
...
\A Sản phẩm hiện tại dùng \texttt{gemini-embedding-001} với \textbf{768 chiều}, lưu ở cột \texttt{vector(768)} (migration v7). Tài liệu/PA ban đầu ghi all-MiniLM-L6-v2 384 chiều --- nhóm đã đổi khi hiện thực để đồng bộ chất lượng embedding với chính nhà cung cấp Gemini (xem Mục 10).
\end{qa}
...
\begin{qa}{50. PA ghi dùng ChromaDB + hybrid search (BM25 + semantic), nhưng sản phẩm dùng pgvector cosine. Vì sao đổi? \tagBoth}
...
\begin{qa}{51. PA nói dùng all-MiniLM-L6-v2 (384 chiều), nhưng code dùng gemini-embedding-001 (768 chiều). Vì sao? \tagBoth}
...
\begin{qa}{52. PA mô tả người dùng ẩn danh theo session\_id và lưu 30 ngày, nhưng sản phẩm có tài khoản đầy đủ. Vì sao? \tagBoth}
...
\begin{qa}{53. PA từng ghi ``Cloud-OCR'', nhưng sản phẩm dùng manga-ocr fine-tune. Vì sao? \tagBoth}
...
\begin{qa}{54. PA nói fallback dịch bằng Google Translate/NLLB-200, còn hiện tại là M2M100/NLLB. Có mâu thuẫn không? \tagBoth}
...
\begin{qa}{55. Phạm vi sản phẩm hiện tại rộng hơn nhiều so với 5 use case trong PA. Vì sao? \tagBoth}
...
\begin{qa}{56. Hạn chế lớn nhất của hệ thống hiện tại? \tagSP}
...
\begin{qa}{57. Lỗi đã biết (known defects) là gì? \tagSP}
...
\begin{qa}{58. Hướng phát triển tiếp theo? \tagSP}
...
\begin{qa}{59. Nếu scale lên hàng chục nghìn người dùng thì kiến trúc chịu được không? \tagSP}
...
\begin{qa}{60. Vấn đề bản quyền manga thì sao? \tagSP}
...
\begin{qa}{61. Chi phí vận hành hiện tại bao nhiêu? \tagSP}
...
\begin{qa}{62. Phần nào của dự án khó nhất? Nhóm học được gì? \tagSP}
...
\begin{qa}{63. Vì sao không dùng một dịch vụ dịch manga có sẵn thay vì tự xây? \tagSP}
...
\begin{qa}{64. Mỗi thành viên đóng góp cụ thể gì? \tagPA}
