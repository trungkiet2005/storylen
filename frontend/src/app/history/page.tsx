      ...
      <div style={{ padding: "40px 56px" }}>
        ...
        {layout === "grid" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
            {filtered.map(s => (
              <div key={s.title} className="stroke-ink panel-shadow" style={{ background: "var(--panel)", overflow: "hidden", cursor: "pointer", transition: "transform 0.12s, box-shadow 0.12s" }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translate(-2px,-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = "6px 6px 0 0 var(--border)"; }}
                  ...
                  <div style={{ flex: 1, padding: 14 }}>
                    ...
                    <div style={{ marginTop: 10 }}>
                      ...
                      <div style={{ height: 6, background: "var(--bg-2)", border: "1px solid var(--border)" }}>
                        <div style={{ width: `${s.progress * 100}%`, height: "100%", background: s.progress === 1 ? "var(--jade)" : "var(--accent)", transition: "width 0.4s" }}/>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    {s.qaReady && (
                      <Link href="/qa" style={{ textDecoration: "none" }} onClick={e => e.stopPropagation()}>
                        <span className="chip" style={{ padding: "2px 8px", fontSize: 10, cursor: "pointer" }}>Q&amp;A</span>
                      </Link>
                    )}
        ...
        {layout === "list" && (
          <div className="stroke-ink" style={{ background: "var(--panel)" }}>
            ...
            {filtered.map((s, i) => (
              <div key={s.title} style={{ display: "grid", gridTemplateColumns: "52px 1fr 120px 120px 100px 140px", padding: "12px 16px", borderBottom: i < filtered.length - 1 ? "1px dashed var(--border-soft)" : "none", alignItems: "center", fontSize: 13 }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <Link href="/reader"><button className="btn btn-sm"><Icon name="book" size={12}/> Đọc</button></Link>
                  {s.qaReady && <Link href="/qa"><button className="btn btn-sm btn-ghost"><Icon name="chat" size={12}/></button></Link>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ marginTop: 28, display: "flex", gap: 20, flexWrap: "wrap" }}>
          {[
            ...
            <div key={stat.label} className="stroke-ink" style={{ background: "var(--panel)", padding: "12px 20px", display: "flex", gap: 12, alignItems: "center" }}>
